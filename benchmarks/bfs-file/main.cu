...
__global__ __launch_bounds__(128, 16) void kernel_first_vertex_step1(uint64_t vertex_count, uint64_t *vertexList, uint64_t num_elems_in_cl, unsigned long long int *winnerList) {
...
__global__ __launch_bounds__(128, 16) void kernel_first_vertex_step2(uint64_t n_cachelines, uint64_t *vertexList, unsigned long long int *winnerList, uint64_t num_elems_in_cl, uint64_t *firstVertexList) {
...
__global__ __launch_bounds__(128, 16) void kernel_verify(uint64_t count, unsigned long long int *list, uint64_t condval, uint8_t type) {
    const uint64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ __launch_bounds__(128, 16) void kernel_frontier_baseline(unsigned int *label, const unsigned int level, const uint64_t vertex_count,
...
__global__ __launch_bounds__(128, 16) void kernel_frontier_coalesce(unsigned int *label, const unsigned int level, const uint64_t vertex_count,
                                                                    const uint64_t *vertexList, const EdgeT *edgeList, const uint64_t curr_frontier_size, unsigned long long int *changed,
                                                                    const uint32_t *curr_frontier, uint32_t *next_frontier) {
    //const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ __launch_bounds__(128, 16) void kernel_frontier_baseline_pc(unsigned int *label, const unsigned int level, const uint64_t vertex_count,
...
__global__ __launch_bounds__(128, 16) void kernel_frontier_coalesce_pc(unsigned int *label, const unsigned int level, const uint64_t vertex_count,
                                                                       const uint64_t *vertexList, array_d_t<uint64_t> *da, const uint64_t curr_frontier_size, unsigned long long int *changed,
                                                                       const uint32_t *curr_frontier, uint32_t *next_frontier) {
    //const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ __launch_bounds__(128, 16) void kernel_frontier_coalesce_ptr_pc(unsigned int *label, const unsigned int level, const uint64_t vertex_count,
                                                                           const uint64_t *vertexList, array_d_t<uint64_t> *da, const uint64_t curr_frontier_size, unsigned long long int *changed,
                                                                           const uint32_t *curr_frontier, uint32_t *next_frontier) {
    //const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ __launch_bounds__(128, 16) void kernel_baseline_pc(array_d_t<uint64_t> *da, uint32_t *label, const uint32_t level, const uint64_t vertex_count,
                                                              const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed, unsigned long long int *globalvisitedcount_d, unsigned long long int *vertexVisitCount_d) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ __launch_bounds__(128, 16) void kernel_coalesce_pc(array_d_t<uint64_t> *da, uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t warpIdx = tid >> WARP_SHIFT;
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1);
    //array_d_t<uint64_t> d_array = *da;
...
__global__ __launch_bounds__(128, 16) void kernel_coalesce_ptr_pc(array_d_t<uint64_t> *da, uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t warpIdx = tid >> WARP_SHIFT;
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1);
    //array_d_t<uint64_t> d_array = *da;
...
__global__ __launch_bounds__(128, 16) void kernel_coalesce_coarse(uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed, uint64_t coarse) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t warpIdx = tid >> WARP_SHIFT;
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1);
...
__global__ __launch_bounds__(128, 16) void kernel_coalesce_coarse_ptr_pc(array_d_t<uint64_t> *da, uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed, uint64_t coarse) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t warpIdx = tid >> WARP_SHIFT;
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1);
    bam_ptr<uint64_t> ptr(da);
...
__global__ __launch_bounds__(128, 16) void kernel_coalesce_hash(uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed, uint64_t stride) {
...
__global__ __launch_bounds__(128, 16) void kernel_coalesce_hash_ptr_pc(array_d_t<uint64_t> *da, uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed, uint64_t stride) {
    const uint64_t oldtid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t oldwarpIdx = oldtid >> WARP_SHIFT;
    const uint64_t laneIdx = oldtid & ((1 << WARP_SHIFT) - 1);
    uint64_t STRIDE = stride;
...
__global__ __launch_bounds__(128, 16) void kernel_coalesce_hash_coarse(uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed, uint64_t coarse, uint64_t stride) {
    const uint64_t oldtid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t oldwarpIdx = oldtid >> WARP_SHIFT;
    const uint64_t laneIdx = oldtid & ((1 << WARP_SHIFT) - 1);
    uint64_t STRIDE = stride;
...
__global__ __launch_bounds__(128, 16) void kernel_coalesce_hash_coarse_ptr_pc(array_d_t<uint64_t> *da, uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed, uint64_t coarse, uint64_t stride) {
    const uint64_t oldtid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t oldwarpIdx = oldtid >> WARP_SHIFT;
    const uint64_t laneIdx = oldtid & ((1 << WARP_SHIFT) - 1);
    uint64_t STRIDE = stride;
...
__global__ __launch_bounds__(128, 16) void kernel_coalesce_hash_half(uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed, uint64_t stride) {
    const uint64_t oldtid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ __launch_bounds__(128, 16) void kernel_coalesce_hash_half_ptr_pc(array_d_t<uint64_t> *da, uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed, uint64_t stride) {
    const uint64_t oldtid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t oldhalfwarpIdx = oldtid >> 4;
    const uint64_t halflaneIdx = oldtid & ((1 << 4) - 1);
    uint64_t STRIDE = stride;
...
__global__ __launch_bounds__(1024, 2) void kernel_coalesce_chunk_pc(array_d_t<uint64_t> *da, uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed) {
    const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t warpIdx = tid >> WARP_SHIFT;
    const uint64_t laneIdx = tid & ((1 << WARP_SHIFT) - 1);
    const uint64_t chunkIdx = warpIdx * CHUNK_SIZE;
...
__global__ __launch_bounds__(128, 16) void kernel_optimized(uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed, uint64_t *first_vertex, uint64_t num_elems_in_cl, uint64_t n_pages, uint64_t stride) {
    //const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
...
//TODO: change launch parameters. The number of warps to be launched equal to the number of cachelines. Each warp works on a cacheline.
//TODO: make it templated.
__global__ __launch_bounds__(128, 16) void kernel_optimized_ptr_pc(array_d_t<uint64_t> *da, uint32_t *label, const uint32_t level, const uint64_t vertex_count, const uint64_t *vertexList, const EdgeT *edgeList, uint64_t *changed, uint64_t *first_vertex, uint64_t num_elems_in_cl, uint64_t n_pages, uint64_t stride, uint64_t iter) {
    //const uint64_t tid = blockDim.x * BLOCK_NUM * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
    const uint64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
